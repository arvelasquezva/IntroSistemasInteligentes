...
public class CuadritoAgentProgram implements AgentProgram{
	...
	public Action compute(Percept p) {
		...
		int sz = Integer.parseInt((String)p.getAttribute(Squares.SIZE));
		if(p.getAttribute(Squares.TURN).equals(color)) {
			...
			for(int i = 0; i < sz; ++i) {
				for(int j  = 0; j < sz; ++j) {
					int sidecnt = 0;
					if(((String)p.getAttribute(i + ":" + j + ":" + Squares.BOTTOM)).equals(Squares.TRUE)) ++sidecnt;
					if(((String)p.getAttribute(i + ":" + j + ":" + Squares.LEFT)).equals(Squares.TRUE)) ++sidecnt;
					if(((String)p.getAttribute(i + ":" + j + ":" + Squares.TOP)).equals(Squares.TRUE)) ++sidecnt;
					if(((String)p.getAttribute(i + ":" + j + ":" + Squares.RIGHT)).equals(Squares.TRUE)) ++sidecnt;
			...
			for(int i = 0; i < sz; ++i) {
				for(int j = 0; j < sz; ++j) {
					...
					boolean bottom = ((String)p.getAttribute((x - 1) + ":" + (y - 1) + ":" + Squares.BOTTOM)).equals(Squares.FALSE);
					boolean left = ((String)p.getAttribute((x - 1) + ":" + (y - 1) + ":" + Squares.LEFT)).equals(Squares.FALSE);
					boolean top = ((String)p.getAttribute((x - 1) + ":" + (y - 1) + ":" + Squares.TOP)).equals(Squares.FALSE);
					boolean right = ((String)p.getAttribute((x - 1) + ":" + (y - 1) + ":" + Squares.RIGHT)).equals(Squares.FALSE);
